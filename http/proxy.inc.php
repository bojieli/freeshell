...
function nginx_conf_gen_for_proxy() {
    ...
    $rs = checked_mysql_query("SELECT http_subdomain, 40x_page, 50x_page, force_ssl, websocket_en, id FROM shellinfo");
    while ($row = mysql_fetch_array($rs)) {
        ...
        $conf .= conf_for_one_freeshell($cname, get_shell_ipv4($row['id']),
            array('httpport' => 80,
                  'page40x' => sanitize_url($row['40x_page']),
                  'page50x' => sanitize_url($row['50x_page']),
                  'force_ssl' => $row['force_ssl'],
                  'websocket_en' => $row['websocket_en']));
    }
    return $conf;
}

function server_block($server_name, $upstream_ip, $additional_conf, $conf) {
    return "
server {
        listen 80;
        listen 443;
...
function conf_for_one_freeshell($cname, $nodeno, $config) {
...
",
    ...
    if ($aggregated_server_name) {
        $retval .= server_block($aggregated_server_name, $nodeno, "", $config);
    }
    return $retval;
}
