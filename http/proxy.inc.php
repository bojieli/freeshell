...
function nginx_conf_gen_for_proxy() {
    ...
    $rs = mysql_query("SELECT http_subdomain, http_cname, nodeno, id FROM shellinfo");
...
function conf_for_one_subdomain($cname_domain, $domain, $nodeno, $httpport) {
    return "
server {
        listen 80;
        listen 443;
...
function subdomain_check($id, $domain) {
    ...
    if ($flag)
        return $flag;
    $rs = mysql_fetch_array(mysql_query("SELECT * FROM shellinfo WHERE `http_subdomain`='$domain' AND id != $id"));
    if ($rs)
        return 4;
    return 0;
}
...
function cname_check($id, $domain) {
    ...
    if ($flag)
        return $flag;
    $rs = mysql_fetch_array(mysql_query("SELECT * FROM shellinfo WHERE `http_cname`='$domain' AND id != $id"));
    if ($rs)
        return 3;
    return 0;
}
