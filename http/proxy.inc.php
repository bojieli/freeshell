...
function nginx_conf_gen_for_proxy() {
    ...
    $rs = checked_mysql_query("SELECT http_subdomain, 40x_page, 50x_page, force_ssl, id FROM shellinfo");
    while ($row = mysql_fetch_array($rs)) {
        ...
        $conf .= conf_for_one_freeshell($cname, get_shell_ipv4($row['id']), 80,
...
function server_block($server_name, $upstream_ip, $additional_conf, $httpport, $page40x, $page50x, $force_ssl) {
    return "
server {
        listen 80;
        listen 443;
        ...
        location / {
                proxy_pass       http://$upstream_ip:$httpport;
